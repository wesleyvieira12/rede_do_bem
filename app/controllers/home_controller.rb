class HomeController < ApplicationController
	...
	def index
		...
		@users_ok = @q.result().where(kind:1, status: "ativo", city_id: @city)

		@categories = @users_ok.joins("INNER JOIN categories c ON users.category_id = c.id").select(:category_id).distinct
